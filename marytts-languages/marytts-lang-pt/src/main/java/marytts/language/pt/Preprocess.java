...
public class Preprocess extends InternalModule {
    ...
    public Preprocess() {
        ...
        rbnf = new RuleBasedNumberFormat(new ULocale("pt"), RuleBasedNumberFormat.SPELLOUT);
        cardinalRule = "%spellout-numbering";
        ordinalRule = getOrdinalRuleName(rbnf);
        yearRule = getYearRuleName(rbnf);
        df = DateFormat.getDateInstance(DateFormat.LONG, new ULocale("pt"));
        try {
    ...
    protected String expand(Document doc) throws ParseException, IOException, MaryConfigurationException {
        ...
        while ((t = (Element) tw.nextNode()) != null) {
            ...
            if (ordinalPattern.matcher(tokenText).matches()) {
                ...
                // first check lexicon
                if (MaryRuntimeUtils.checkLexicon("pt_BR", tokenText).length == 0) {
    ...
    protected String expandDate(String date) throws ParseException {
        ...
        if (Integer.parseInt(dateParts[0]) > 12) {
            humanDate = DateFormat.getPatternInstance("dd.MM.yyyy", new ULocale("pt")).parse(date);
        } else {
